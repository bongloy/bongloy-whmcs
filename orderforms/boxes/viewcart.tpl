    ...
    {else}
        ...
        <form method="post" action="{$smarty.server.PHP_SELF}?a=checkout" id="mainfrm">
            ...
            <div class="text-center">
                ...
                <div class="line-padded">
                    <button type="submit" id="btnCompleteOrder" class="btn btn-primary btn-lg submitbutton ordernow" {if $cartitems==0} disabled="disabled"{/if} onclick="this.value='{$LANG.pleasewait}'"{if $custtype eq "existing" && !$loggedin} formnovalidate{/if}>{$LANG.completeorder} &nbsp;<i class="fa fa-arrow-circle-right"></i></button>
                </div>
